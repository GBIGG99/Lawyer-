...
import { Loader2, X, BrainCircuit, User, Gavel, Globe, FileText, AlertTriangle, Calendar, Download, ChevronDown } from 'lucide-react';
...
const getNodeIcon = (type: string, className = "w-5 h-5") => {
    switch (type) {
        ...
        case 'location': return <Globe className={className} />;
        case 'institution': return <Gavel className={className} />;
...
const GraphView: React.FC<{ 
    ...
    useEffect(() => {
        ...
        const iterations = 200; // Increased iterations for stability
        const nodeRadius = 25; // Base visual radius
        const collisionRadius = 70; // Buffer distance to prevent overlap including labels
        
        // Optimal distance k
        const k = Math.sqrt((width * height) / filteredNodes.length) * 0.9;
        let temp = 25; // Initial temperature
        ...
        for (let iter = 0; iter < iterations; iter++) {
            ...
            for (let i = 0; i < filteredNodes.length; i++) {
                for (let j = 0; j < filteredNodes.length; j++) {
                    ...
                    // Standard repulsion
                    const fr = (k * k) / dist;
                    displacements[u.id].dx += (dx / dist) * fr;
                    displacements[u.id].dy += (dy / dist) * fr;

                    // Hard Collision Detection
                    if (dist < collisionRadius) {
                        const overlapPush = (collisionRadius - dist) * 1.5;
                        displacements[u.id].dx += (dx / dist) * overlapPush;
                        displacements[u.id].dy += (dy / dist) * overlapPush;
                    }
                }
            }

            // 2. Attraction (Spring-like)
            filteredLinks.forEach(link => {
                const uId = link.source, vId = link.target;
                if (!currentPos[uId] || !currentPos[vId]) return;
                ...
                const dxNorm = (dx / dist) * fa;
                const dyNorm = (dy / dist) * fa;
            ...
            // 3. Central Gravity (Keep clusters centered)
            filteredNodes.forEach(node => {
                const dx = currentPos[node.id].x - width / 2;
                const dy = currentPos[node.id].y - height / 2;
                const dist = Math.sqrt(dx * dx + dy * dy) || 0.01;
                const gravity = 0.05;
                displacements[node.id].dx -= dx * gravity;
                displacements[node.id].dy -= dy * gravity;
            });

            // 4. Update Positions with Temperature Constraint
            filteredNodes.forEach(node => {
                ...
                currentPos[node.id].x = Math.max(50, Math.min(width - 50, currentPos[node.id].x));
                currentPos[node.id].y = Math.max(50, Math.min(height - 50, currentPos[node.id].y));
            });
            ...
            <div className="fixed inset-0 bg-[#050505]/95 backdrop-blur-md flex justify-center items-center z-50 p-4">
                <div className="bg-[#121212] border border-[#6e501a] p-10 max-w-xl w-full text-center relative shadow-2xl">
                    ...
                    <label htmlFor="f-mapper-manual" className="block p-12 border border-dashed border-[#6e501a]/40 mb-10 cursor-pointer hover:bg-[#b5892f]/5 transition-colors group">
                        <BrainCircuit className="w-16 h-16 text-[#6e501a] mx-auto mb-6 group-hover:text-[#b5892f] transition-colors" />
    ...
    if (isLoading) return (
        <div className="fixed inset-0 bg-[#050505] flex flex-col items-center justify-center z-50 p-8 text-center">
            <div className="relative mb-12">
                <Loader2 className="w-24 h-24 text-[#b5892f] animate-spin" />
                <div className="absolute inset-0 flex items-center justify-center">
                    <BrainCircuit className="w-8 h-8 text-[#b5892f] animate-pulse" />
                </div>
            </div>
        ...
        <div className="fixed inset-0 bg-[#050505] z-50 flex flex-col overflow-hidden animate-[fadeIn_0.3s_ease-out]">
            <header className="p-4 lg:p-6 bg-[#121212] border-b border-[#6e501a]/30 flex justify-between items-center z-50">
                <div className="flex items-center gap-3 lg:gap-6">
                    <BrainCircuit className="w-6 h-6 lg:w-8 lg:h-8 text-[#b5892f]" />
                ...
                <div className="flex gap-2 lg:gap-4 relative">
                    <div className="relative">
                      ...
                      >
                          {isExporting ? <Loader2 className="w-3 h-3 animate-spin" /> : <Download className="w-4 h-4" />}
                    ...
                    <button onClick={onClose} className="p-2 text-[#6e501a] hover:text-[#efefef]"><X className="w-6 h-6" /></button>
                </div>
            </header>
